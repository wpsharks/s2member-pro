...
if(!class_exists('c_ws_plugin__s2member_pro_menu_page_stripe_ops'))
{
	...
	class c_ws_plugin__s2member_pro_menu_page_stripe_ops
	{
		public function __construct()
		{
			...
			echo '<p><a href="http://www.s2member.com/authorize.net" target="_blank" rel="external">Stripe</a> is a leading provider of payment gateway services, managing the submission of billions of transactions to processing networks on behalf of merchant customers. Stripe is a solution offered by the CyberSource Corporation, a wholly owned subsidiary of Visa (NYSE: V).</p>'."\n";
			echo '<p>s2Member has been integrated with Stripe for Direct Payments and also for ARB (Automated Recurring Billing). In order to take advantage of this integration, you will need to have an Stripe Merchant Account. Once you have an account, all of the details below can be obtained from inside of your Stripe account. If you need assistance, please check their <a href="http://www.s2member.com/authorize.net-developers" target="_blank" rel="external">help section</a>.</p>'."\n";
			echo '<p><em><strong>*Stripe Version (3.1)*</strong> s2Member integrates with Transaction Version 3.1 for Stripe. Please log into your Stripe Merchant account and make sure your Transaction Version setting is configured as: <code>3.1</code>. You will find this inside your Stripe Merchant account, under: <code>Account -› Settings -› Transaction Version</code>.</em></p>'."\n";
			echo '<p><em><strong>*Recurring Billing*</strong> If you plan to use any of the ( `Subscription` ) options in the s2Member Form Generator for Stripe, you will ALSO need <a href="http://www.s2member.com/authorize.net-arb" target="_blank" rel="external">ARB (Automated Recurring Billing)</a> enabled for your Stripe account. Stripe\'s Recurring Billing service is <strong>required</strong> for all types of ( `Subscriptions` ), whether you intend for them to be recurring or not. However, it is NOT required for ( `Buy Now` ) functionality. The drop-down menus in the s2Member Form Generator, have been marked ( `Subscription` ) and ( `Buy Now` ) just for this reason. See: <code>s2Member -› Stripe Pro Forms</code>. This way you can see which options will require the use of Stripe\'s Recurring Billing service. Stripe will charge you a small monthly fee for their Automated Recurring Billing service.</em></p>'."\n";
			...
			echo '<p>The Stripe ARB (Automated Recurring Billing) service will assist in notifying s2Member whenever a Subscription expires. For example, if a Customer cancels their own Subscription; or if you cancel a Customer\'s Subscription through Stripe, s2Member will eventually be notified (with a 24-48 hour delay), and the account for that Customer will either be demoted to a Free Subscriber, or deleted automatically (based on your configuration). ~ Otherwise, under normal circumstances, s2Member will not process an EOT until the User has completely used up the time they paid for.</em></p>'."\n";
			...
			echo '</select><br />'."\n";
			echo '<em>N/A with Stripe. Stripe does NOT notify s2Member on refunds/chargebacks.</em>'."\n";
			echo '</td>'."\n";
