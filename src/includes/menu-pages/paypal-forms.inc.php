...
if(!class_exists("c_ws_plugin__s2member_pro_menu_page_paypal_forms"))
{
	...
	class c_ws_plugin__s2member_pro_menu_page_paypal_forms
	{
		public function __construct()
		{
			...
			do_action("ws_plugin__s2member_during_paypal_forms_page_before_left_sections", get_defined_vars());
			...
			if(!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site())
			{
				...
				//260913.2356 Document success="" consistently for successful checkout, billing updates, and cancellations.
				echo '<p>s2Member Pro opens the door for Custom Return URLs on success. You can add a special attribute to any Form Shortcode (<code>success="/my-thank-you-page/"</code>). This can redirect after a successful checkout, billing update, or cancellation, as applicable to the Form, and can include supported Replacement Code variables. For example, (<code>success="/my-thank-you-page/?subscr_id=%%subscr_id%%"</code>).</p>'."\n";
				...
				echo '<p>For a Cancellation Form, <code>success=""</code> is used only after s2Member confirms that the Subscription was cancelled. If s2Member cannot complete the cancellation directly and the Member must finish it in PayPal\'s subscription-management interface, s2Member cannot redirect after that external action. Billing Update Forms use <code>success=""</code> after the update completes successfully.</p>'."\n";
				echo '<ul style="margin: 10px 0 10px 20px; list-style: disc outside;" class="ws-menu-page-li-margins">'."\n";
			...
			echo (!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site()) ? '<li><code>success=""</code> 100% optional. Creates a Custom Return URL after a successful checkout, billing update, or cancellation, as applicable to the Form. For cancellations, s2Member redirects only after it confirms the Subscription was cancelled. Please see the section above titled: <em>Custom Return URLs on Success</em>.</li>'."\n" : '';
			echo (!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site()) ? '<li><code>template=""</code> 100% optional. This can be a custom template file inside <code>WP_CONTENT_DIR</code>, including your WordPress theme directory. Custom filenames must be approved under <strong>s2Member → General Options → Pro Shortcode Templates Whitelist</strong>. Standard PayPal Pro-Form template filenames are allowed automatically. For example: <code>template="checkout.php"</code>. Please see <a href="http://s2member.com/kb-article/can-i-customize-pro-forms/" target="_blank" rel="external">this KB article</a> for further details.</li>'."\n" : ''; //260812 Mention whitelist.
			...
			echo '<pre class="code">';
			echo '<code>';
			echo '[s2Member-Pro-PayPal-Form placeholder="Please choose..."]'."\n";
