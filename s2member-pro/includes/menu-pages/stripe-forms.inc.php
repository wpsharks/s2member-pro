				...
				public function __construct()
					{
						...
						echo '<p>The s2Member Pro Module makes it possible for s2Member to use <a href="http://www.s2member.com/authorize.net" target="_blank" rel="external">Stripe</a> Pro Forms. Pro Forms integrate seamlessly with WordPress Shortcodes. This allows you to keep Customers on your site at all times, and it consolidates the Checkout / Registration steps into a single form that you can dress up just the way you like. If you would like to take advantage of Stripe integration, please supply your Stripe API Login ID, Transaction Key, and Secret MD5 Hash. See <code>s2Member -› Stripe Options</code>.</p>'."\n";
						echo '<p><em><strong>*Recurring Billing*</strong> If you plan to use any of the ( `Subscription` ) options in the s2Member Form Generators below, you will ALSO need <a href="http://www.s2member.com/authorize.net-arb" target="_blank" rel="external">ARB (Automated Recurring Billing)</a> enabled for your Stripe account. Stripe\'s Recurring Billing service is <strong>required</strong> for all types of ( `Subscriptions` ), whether you intend for them to be recurring or not. However, it is NOT required for ( `Buy Now` ) functionality. The drop-down menus in the Form Generators below, have been marked ( `Subscription` ) and ( `Buy Now` ) just for this reason. This way you can see which options will require the use of Stripe\'s Recurring Billing service. Stripe will charge you a small monthly fee for their Automated Recurring Billing service.</em></p>'."\n";
						...
						for($n = 1; $n <= $GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["levels"]; $n++)
							{
								...
								echo '<p>Very simple. All you do is customize the form fields provided, for each Membership Level that you plan to offer. Then press (Generate Form Code). These special Stripe Forms are customized to work with s2Member seamlessly. Member accounts will be activated instantly, in an automated fashion. When a Member cancels their Membership, or fails to make payments on time, s2Member will automatically terminate their Membership privileges. s2Member makes extensive use of the Stripe Silent Post and ARB services. s2Member receives updates from Stripe behind-the-scene.</p>'."\n";
						...
						if(!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site())
							{
								...
								echo '<p>I want to charge: $<input type="text" autocomplete="off" id="ws-plugin--s2member-pro-ccap-amount" value="0.01" size="4" /> / <select id="ws-plugin--s2member-pro-ccap-term">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/stripe-membership-ccap-terms.php"))).'</select></p>'."\n";
						...
						echo '<p>I want to charge: $<input type="text" autocomplete="off" id="ws-plugin--s2member-pro-sp-amount" value="0.01" size="4" /> / <select id="ws-plugin--s2member-pro-sp-hours">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/stripe-sp-hours.php"))).'</select></p>'."\n";
						...
						echo '<li><code>cc="USD"</code> 3 character Currency Code. Most Stripe accounts process all transactions in <code>USD</code> (by default). However, Stripe has recently introduced support for <code>CAD</code>, <code>EUR</code> and/or <code>GBP</code> currency codes (perhaps others are coming in the near future). If your Stripe account supports one of these additional currency codes, you can change this if you like. However, please be sure to consult with Stripe and confirm which currency that your merchant account is billing in. Not valid when <code>cancel="1"</code>.</li>'."\n";
