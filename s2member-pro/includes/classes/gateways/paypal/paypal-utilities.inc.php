		...
		public static function paypal_apply_coupon($attr = array(), $coupon_code = '', $return = '', $process = array())
		{
			if(($coupon_code = trim(strtolower($coupon_code))) || ($coupon_code = trim(strtolower($attr['coupon']))))
				if($attr['accept_coupons'] && $GLOBALS['WS_PLUGIN__']['s2member']['o']['pro_coupon_codes'])
				{
					...
					unset($affiliate_suffix_chars, $m); // Housekeeping.
					...
					foreach(c_ws_plugin__s2member_utils_strings::trim_deep(preg_split('/['."\r\n\t".']+/', $GLOBALS['WS_PLUGIN__']['s2member']['o']['pro_coupon_codes'])) as $_line)
					{
						if(($_line = trim($_line, " \r\n\t\0\x0B|")) && is_array($_coupon = preg_split('/\|/', $_line)))
						{
							...
							$coupon['expired'] = (!empty($_coupon[2]) && strtotime($_coupon[2]) < time()) ? $_coupon[2] : FALSE; // Expired?
			...
			$attr['_coupon_applies']      = (isset($coupon_applies) && $coupon_applies) ? '1' : '0'; // Coupon applies?
			$attr['_coupon_code']         = (isset($coupon_applies) && $coupon_applies) ? $coupon_code : ''; // Coupon applies?
