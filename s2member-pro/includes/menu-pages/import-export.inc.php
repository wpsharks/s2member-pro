...
if(!class_exists("c_ws_plugin__s2member_pro_menu_page_import_export"))
{
	...
	class c_ws_plugin__s2member_pro_menu_page_import_export
	{
		public function __construct()
		{
			...
			$enable_old_versions = !empty($_REQUEST['enable_old_versions']);

			if(!$enable_old_versions)
				echo '<a href="'.esc_attr(add_query_arg('enable_old_versions', '1')).'">click here to ENABLE the older versions</a>';
			else echo '<a href="'.esc_attr(remove_query_arg('enable_old_versions')).'">click here to HIDE the older versions</a>';
			...
			if($enable_old_versions)
			{
				...
				if(is_multisite() && c_ws_plugin__s2member_utils_conds::is_multisite_farm() && !is_main_site())
				{
					echo '<div class="ws-menu-page-group" title="Old Version :: User/Member CSV Importation"'.((isset($_POST["ws_plugin__s2member_pro_import_v1_users"])) ? ' default-state="open"' : '').'>'."\n";
					...
					echo '<td>'."\n";
					echo '<form method="post" enctype="multipart/form-data" name="ws_plugin__s2member_pro_import_users_form" id="ws-plugin--s2member-pro-import-users-form">'."\n";
					echo '<input type="hidden" name="ws_plugin__s2member_pro_import_v1_users" id="ws-plugin--s2member-pro-import-users" value="'.esc_attr(wp_create_nonce("ws-plugin--s2member-pro-import-users")).'" />'."\n";
			...
			if($enable_old_versions)
			{
				 ...
				 * Old version.
				 */
				echo '<div class="ws-menu-page-group" title="Old Version :: User/Member CSV Exportation">'."\n";

				echo '<div class="ws-menu-page-section ws-plugin--s2member-pro-user-exportation-section">'."\n";
				echo '<h3>User/Member Exportation (download CSV export files)</h3>'."\n";

				echo '<form method="post" name="ws_plugin__s2member_pro_export_users_form" id="ws-plugin--s2member-pro-export-users-form">'."\n";
				echo '<input type="hidden" name="ws_plugin__s2member_pro_export_v1_users" id="ws-plugin--s2member-pro-export-users" value="'.esc_attr(wp_create_nonce("ws-plugin--s2member-pro-export-users")).'" />'."\n";
			...
			if($enable_old_versions)
			{
				 ...
				 * Old version.
				 */
				echo '<div class="ws-menu-page-group" title="Old Version :: s2Member Options (Import/Export)"'.((isset ($_POST["ws_plugin__s2member_pro_import_v1_ops"])) ? ' default-state="open"' : '').'>'."\n";

				echo '<div class="ws-menu-page-section ws-plugin--s2member-pro-ops-importation-exportation-section">'."\n";

				echo '<h3 style="margin-bottom:5px;">s2Member Options Export <small>(<a href="'.esc_attr(site_url("/?ws_plugin__s2member_pro_export_v1_ops=".urlencode(wp_create_nonce("ws-plugin--s2member-pro-export-ops")))).'">download serialized export file</a>)</small></h3>'."\n";
				echo '<p style="margin-top:5px;">This allows you to export your current s2Member configuration, and then import it into another instance of WordPress.'."\n";
				...
				echo '<td>'."\n";
				echo '<form method="post" enctype="multipart/form-data" name="ws_plugin__s2member_pro_import_ops_form" id="ws-plugin--s2member-pro-import-ops-form">'."\n";
				echo '<input type="hidden" name="ws_plugin__s2member_pro_import_v1_ops" id="ws-plugin--s2member-pro-import-ops" value="'.esc_attr(wp_create_nonce("ws-plugin--s2member-pro-import-ops")).'" />'."\n";
				echo '<input type="file" name="ws_plugin__s2member_pro_import_ops_file" id="ws-plugin--s2member-pro-import-ops-file" />&nbsp;&nbsp;&nbsp;<input type="submit" value="Import Now" style="font-size:120%; font-weight:normal;" />'."\n";
				echo '</form>'."\n";
				echo '</td>'."\n";
