...
if(!class_exists("c_ws_plugin__s2member_pro_menu_page_paypal_button_attrs"))
{
	...
	class c_ws_plugin__s2member_pro_menu_page_paypal_button_attrs
	{
		public function __construct()
		{
			...
			echo '<li><code>success=""</code> Success Return URL <em>(optional)</em>. s2Member handles this automatically for you. However, if you would prefer to take control over the landing page after checkout <em>(i.e., your own custom Thank-You Page)</em>, you can. If supplied, this must be a full URL, starting with <code>http://</code>. Note, s2Member will NOT use this value if an existing account holder is being modified. s2Member handles account updates <em>(i.e., billing modification)</em> in a more dynamic way. Your Success Return URL is applied to (new) Customers, and also after a successful PayPal Checkout cancellation when using <code>cancel="1" output="button"</code>. For cancellation buttons, the Success URL must normally be on the same site; additional hosts can be allowed with WordPress\'s <code>allowed_redirect_hosts</code> filter.</li>'."\n";
