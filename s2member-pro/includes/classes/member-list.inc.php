...
if(!class_exists("c_ws_plugin__s2member_pro_member_list"))
	{
		...
		{
			/**
			 * @var string Default page var.
			 */
			public static $default_page_var = "s2-p";
			...
			public static function query($args = array())
				{
					...
					$page_var = apply_filters("ws_plugin__s2member_pro_member_list_page_var", self::$default_page_var);
					if(empty($_REQUEST[$page_var]) || ($page = (integer)$_REQUEST[$page_var]) < 1)
					...
					return array("query" => $query, "pagination" => self::paginate($page, (integer)$query->get_total(), $args["number"], $page_var));
				}
			 ...
			 * @param string  $page_var The variable to use as the page var.
			 ...
			 */
			protected static function paginate($current_page, $total_results, $per_page, $page_var, $current_url = "", $pagination_limit = 10)
				{
					$current_page  = max(1, (integer)$current_page);
					$total_results = max(0, (integer)$total_results);
					$per_page      = max(1, (integer)$per_page);
					$page_var      = (!$page_var) ? self::$default_page_var : (string)$page_var;
					...
					$current_url = remove_query_arg($page_var, $current_url);
							   ...
							   || $_i >= $current_page - $pagination_limit || $_i <= $current_page + $pagination_limit
							) // First or last; or within the current pagination limit.
								{
									if($_i === $current_page)
										...
										{
											$pagination[$_i]["text"] = (string)$_i;
											$pagination[$_i]["url"]  = add_query_arg($page_var, $_i, $current_url);
											$pagination[$_i]["link"] = '<a href="'.esc_attr(add_query_arg($page_var, $_i, $current_url)).'">'.(string)$_i.'</a>';
										}
									$_show_dots = TRUE;
								}
							else if($_show_dots)
