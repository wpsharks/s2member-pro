...
if(!class_exists("c_ws_plugin__s2member_pro_menu_page_alipay_buttons"))
{
	...
	class c_ws_plugin__s2member_pro_menu_page_alipay_buttons
	{
		public function __construct()
		{
			...
			echo '<p><select id="ws-plugin--s2member-pro-sp-link-hours">'.trim(c_ws_plugin__s2member_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/alipay-sp-hours.php"))).'</select> <input type="button" value="Generate Access Link" onclick="ws_plugin__s2member_pro_alipaySpLinkGenerate();" /> <img id="ws-plugin--s2member-pro-sp-link-loading" src="'.esc_attr($GLOBALS["WS_PLUGIN__"]["s2member"]["c"]["dir_url"]).'/src/images/ajax-loader.gif" alt="" style="display:none;" /></p>'."\n";
