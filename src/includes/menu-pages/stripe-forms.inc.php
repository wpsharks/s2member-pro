...
if(!class_exists('c_ws_plugin__s2member_pro_menu_page_stripe_forms'))
{
	...
	class c_ws_plugin__s2member_pro_menu_page_stripe_forms
	{
		public function __construct()
		{
			...
			// echo '<li><code>collect_billing_address=""</code> By default, this is set to your global default option for this behavior. However, if you want to override your global default configuration for a specific Pro-Form, you can set this to either <code>1</code> (collect) or <code>0</code> (do NOT collect). See also: <strong>s2Member → Stripe Options → Account Details → Stripe Should Collect a Full Billing Address?</strong>; i.e., your global default option.</li>'."\n";
			// echo '<li><code>collect_shipping_address=""</code> By default, this is set to your global default option for this behavior. However, if you want to override your global default configuration for a specific Pro-Form, you can set this to either <code>1</code> (collect) or <code>0</code> (do NOT collect). See also: <strong>s2Member → Stripe Options → Account Details → Stripe Should Collect a Full Shipping Address?</strong>; i.e., your global default option.</li>'."\n";
			...
			echo '<li><code>pform="offer"</code> Allows restricting a coupon to specific pro-forms. Enter a unique name for this pro-form, e.g., <code>pform="newoffer"</code>. In the Coupons table, a new "Pro-Forms" column lets you specify one or more pro-form names (comma-separated). Coupons limited to specific pro-forms will only work on those, while coupons without a specified pro-form will work on any pro-form that accepts coupons.</li>'."\n";
			...
			echo (!is_multisite() || !c_ws_plugin__s2member_utils_conds::is_multisite_farm() || is_main_site()) ? '<li><code>template=""</code> 100% optional. This can be a custom template file inside <code>WP_CONTENT_DIR</code>, including your WordPress theme directory. Custom filenames must be approved under <strong>s2Member → General Options → Pro Shortcode Templates Whitelist</strong>. Standard Stripe Pro-Form template filenames are allowed automatically. For example: <code>template="checkout.php"</code>. Please see <a href="http://s2member.com/kb-article/can-i-customize-pro-forms/" target="_blank" rel="external">this KB article</a> for further details.</li>'."\n" : ''; //260812 Mention whitelist.
			...
			echo '<li><code>validate_zipcode=""</code> By default, this is set to your global default option for this behavior. However, if you want to override your global default configuration for a specific Pro-Form, you can set this to either <code>1</code> (validate) or <code>0</code> (do NOT validate). See also: <strong>s2Member → Stripe Options → Account Details → Stripe Should Verify Zipcodes?</strong>; i.e., your global default option.</li>'."\n";
			echo '</ul>'."\n";
			echo '</td>'."\n";
			...
			echo '<pre class="code">';
			echo '<code>';
			echo '[s2Member-Pro-Stripe-Form placeholder="Please choose..."]'."\n";
