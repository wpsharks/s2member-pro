...
if(!function_exists('ws_plugin__s2member_pro_default_options'))
{
	...
	function ws_plugin__s2member_pro_default_options($default_options = array())
	{
		$pro_default_options = array( // Defaults for the Pro Module.
		                              ...
		                              'pro_stripe_api_publishable_key'          => '', 'pro_stripe_api_login_id' => '', 'pro_stripe_api_trans_key' => '', 'pro_stripe_api_salt_key' => '', 'pro_stripe_sandbox' => '0',
		...
		*/
		...
		foreach($options as $key => &$value /* $GLOBALS['WS_PLUGIN__']['s2member']['o'] */)
		{
			if(isset($pro_default_options[$key]) || preg_match('/^pro_/', $key))
			{
				...
				else if(preg_match('/^pro_stripe_(?:api_publishable_key|api_login_id|api_trans_key|api_salt_key)$/', $key) && (!is_string($value) || !strlen($value)))
					$value = $pro_default_options[$key];
